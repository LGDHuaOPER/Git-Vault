---
title: "GetEvaluationRun - 获取评估运行详情"
source: "https://help.aliyun.com/zh/document_detail/3045677.html?spm=a2c4g.11186623.help-menu-3033820.d_6_1_3_5_0_6.13c93909OHN4TC"
author:
  - "阿里云计算"
published:
created: 2026-07-23
...
tags:
  ...
  - "官方文档"
"word-count": "466"
更新时间: "2026-07-08 11:36:45"
---
获取评估运行详情

## 调试

[您可以在OpenAPI Explorer中直接运行该接口，免去您计算签名的困扰。运行成功后，OpenAPI Explorer可以自动生成SDK代码示例。](https://api.aliyun.com/api/AgentLoop/2026-05-20/GetEvaluationRun)[您可以在OpenAPI Explorer中直接运行该接口，免去您计算签名的困扰。运行成功后，OpenAPI Explorer可以自动生成SDK代码示例。](https://api.alibabacloud.com/api/AgentLoop/2026-05-20/GetEvaluationRun)

 [![](https://img.alicdn.com/tfs/TB16JcyXHr1gK0jSZR0XXbP8XXa-24-26.png) 调试](https://api.aliyun.com/api/AgentLoop/2026-05-20/GetEvaluationRun)
...
```
GET /api/v1/evaluation-task/{agentSpace}/{taskId}/run/{runId} HTTP/1.1
```
...
| runId | string | 否   | 评估运行 ID。 | eval-run-4fd47f3d7e684e15b1d3d178c6a5b81a |
...
| evaluatorProgress | array<object> | 评估器维度进度列表。 | \\[{"evaluatorName":"Builtin.agent\\_task\\_completion","totalCount":100,"successCount":96,"failedCount":4}\\] |
|     | object | 单个评估器进度对象。 | {"evaluatorName":"Builtin.agent\\_task\\_completion"} |
| evaluatorName | string | 评估器名称。 | Builtin.agent\\_task\\_completion |
| totalCount | integer | 该评估器总条数。 | 100 |
| successCount | integer | 该评估器成功条数。 | 96  |
| failedCount | integer | 该评估器失败条数。 | 4   |
| createdAt | integer | 创建时间，秒级 Unix 时间戳。 | 1782816000 |
| updatedAt | integer | 更新时间，秒级 Unix 时间戳。 | 1782816600 |
| evaluators | string | 运行创建时的评估器配置快照，JSON 字符串。 | \\[{"evaluatorRef":"Builtin.agent\\_task\\_completion"}\\] |
| dataStartTime | integer | 运行数据窗口起始时间，秒级 Unix 时间戳。 | 1782816000 |
| dataEndTime | integer | 运行数据窗口结束时间，秒级 Unix 时间戳。 | 1782902400 |
...
更多信息，参考[变更详情](https://api.aliyun.com/document/AgentLoop/2026-05-20/GetEvaluationRun#workbench-doc-change-demo)[变更详情](https://api.alibabacloud.com/document/AgentLoop/2026-05-20/GetEvaluationRun#workbench-doc-change-demo)。