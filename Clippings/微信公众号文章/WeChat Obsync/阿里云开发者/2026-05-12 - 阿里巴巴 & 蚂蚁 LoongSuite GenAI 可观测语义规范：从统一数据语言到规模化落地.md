---
source_url: "https://mp.weixin.qq.com/s?__biz=MzIzOTU0NTQ0MA==&mid=2247559954&idx=1&sn=0bb0ae4137b4f734ca75997db54e418a&chksm=e893450870ca6dbd11b5a2b16fc4de321d8b1e9473bede4bce422709727001150107fd638d8d&mpshare=1&scene=1&srcid=0513aTCmaT6oCrsnHanbgiPp&sharer_shareinfo=1b4b1e3dabee6f859ad3b33f0d31a74a&sharer_shareinfo_first=1b4b1e3dabee6f859ad3b33f0d31a74a"
title: "阿里巴巴 & 蚂蚁 LoongSuite GenAI 可观测语义规范：从统一数据语言到规模化落地"
account: "阿里云开发者"
published_at: "2026-05-12T10:00:00.000Z"
saved_at: "2026-07-15T16:11:39.824Z"
sync_id: "art_a4941a1a6ecb41279bf45c476666abea"
parse_status: "ok"
---

# 阿里巴巴 & 蚂蚁 LoongSuite GenAI 可观测语义规范：从统一数据语言到规模化落地

![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_1.jpg)
...
![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_2.png)
...
| 属性 | 说明 | 状态 |
| --- | --- | --- |
| `ENTRY` | AI应用系统的入口点调用标识符 | Development |
| `STEP` | ReAct迭代步骤标识符 | Development |

#### 实现效果

目前该语义规范已经在多个 Agent 场景中落地，包括 OpenClaw, QwenPaw, Hermes Agent，以下是在 OpenClaw 场景下实现语义规范，接入后的效果：

![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_3.png)
...
| **属性** | **类型** | **说明** | **示例值** |
| --- | --- | --- | --- |
| `gen_ai.skill.name` | string | Skill 名称 | add_to_cart |
| `gen_ai.skill.id` | string | Skill 实例标识，区分灰度/A/B 实验 | skill_7d1af98c |
| `gen_ai.skill.description` | string | Skill 功能描述 | Comprehensive reference for GitHub CLI |
| `gen_ai.skill.version` | string | Skill 版本号 | 1.0.0 |

当前阶段，这些属性附着在已有的  `execute_tool`  Span 上，无需引入新 Span 类型即可快速落地。

同时，基于集团业务，我们落地了独立  `invoke_skill`  Span 的方案，并向 OTel 社区提交了提案
...
![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_4.png)
...
| 属性 | 类型 | 描述 | 示例值 |
| --- | --- | --- | --- |
| `gen_ai.response.per_token_time_to_schedule` | double[] | 每个生成 Token 进入迭代的时间戳（相对于 Span/请求的开始时间）。每个生成的 Token 对应一个值，以秒为单位。 | [0.010, 0.007, 0.006] |
| `gen_ai.response.per_token_time_to_generate` | double[] | 每个 Token 出迭代的时间戳 (相对于 Span/请求的开始)。每个生成的 Token 一个值，以秒为单位。 | [0.0031, 0.0028, 0.0029] |
| `gen_ai.iteration.per_token_batch_size` | int[] | 每个 Token 计算所在迭代批的总请求数目。每个生成的 Token 对应一个值。 | [32, 32, 28] |
| `gen_ai.iteration.per_token_cumulative_count` | int[] | 每个 Token 计算所在迭代批的总 Token 数目。每个生成的 Token 对应一个值。 | [4096, 4097, 4098] |
...
| 属性 | 类型 | 描述 | 示例值 |
| --- | --- | --- | --- |
| `gen_ai.response.candidate.per_position_decoded_tokens` | string[][] | 对于每个生成的 Token 位置，top-k候选 Token 字符串的列表。外部数组索引生成的 Token 位置。内部数组包含该位置的候选top-k Token 字符串，第一个是被选中的 Token。 | [["Hello", "Hello","Hi"], ["there", "world","there"]] |
| `gen_ai.response.candidate.per_position_token_ids` | int[][] | 对于每个生成的 Token 位置，top-k候选 Token id 的列表。外部数组索引生成的 Token 位置; 内部数组包含该位置的候选top-k token id，第一个是被选中的 Token。 | [[15043, 15043, 1723], [1052, 995, 1052]] |
| `gen_ai.response.candidate.per_position_logprobs` | double[][] | 对于每个生成的 Token 位置，top-k候选 Token logits 的列表。外部数组索引生成的 Token 位置; 内部数组包含该位置的候选top-k token logits，第一个是被选中的 Token。 | [[-1.03, -0.12, -1.03], [-0.05, -0.05, -2.11]] |
...
引擎 Token 分析的 Token 粒度性能数据可以揭示有哪些慢 Token？引擎并发分析进一步解答这些 Token 为什么慢？另外 Token 粒度的概率分布数据可以揭示异常 Token 的大模型输出是否有正常还是采样参数设置不合理。产品上线后，历经年底大促，在稳定性战场成功帮助引擎/SRE/业务同学定位多起稳定性问题，10倍提速问题定界效率，真正做到了又快又准，并且进一步给出了优化建议。下面选择了一些比较典型的案例来阐述产品功能与业务价值。
...
![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_5.png)
...
![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_6.png)
...
![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_7.png)
...
1. LoongSuite Python 对应实现 LoongSuite-utils-genai：https://pypi.org/project/loongsuite-util-genai/

2. LoongSuite JS 对应实现 LoongSuite-utils-genai：https://www.npmjs.com/package/@loongsuite/opentelemetry-util-genai

**架构设计**

GenAI Utils 的整体架构遵循 "分层解耦、统一收口" 的设计原则：

![](附件资源/阿里巴巴%20&%20蚂蚁%20LoongSuite%20GenAI%20可观测语义规范：从统一数据语言到规模化落地/img_8.png)
...
| **操作类型** | **Invocation 数据类** | **Handler 方法** | **Span Name 示例** |
| --- | --- | --- | --- |
| LLM 调用 | LLMInvocation | handler.llm() | chat qwen-max |
| Agent 调用 | InvokeAgentInvocation | handler.invoke_agent() | invoke_agent ShoppingAssistant |
| Agent 创建 | CreateAgentInvocation | handler.create_agent() | create_agent SupportAgent |
| 工具执行 | ExecuteToolInvocation | handler.execute_tool() | execute_tool get_weather |
| 向量嵌入 | EmbeddingInvocation | handler.embedding() | embeddings text-embedding-3-small |
| 文档检索 | RetrieveInvocation | handler.retrieve() | retrieve_documents |
| 文档重排序 | RerankInvocation | handler.rerank() | rerank_documents |
| 记忆操作 | MemoryInvocation | handler.memory() | memory_operation search |
...
在上面这段代码中，开发者没有直接操作任何 OTel API——不需要手动创建 Span、设置 SpanKind、挂载 gen_ai.agent.name 属性、记录 Duration Metrics——这些全部由 ExtendedTelemetryHandler 在 Context Manager 的 __enter__ 和 __exit__ 中自动完成。若在调用过程中抛出异常，Handler 会自动捕获并在 Span 上设置 error.type 属性和错误状态。详细使用过程可以参考文档：
...
| 框架 | 覆盖场景 |
| --- | --- |
| DashScope（通义千问） | LLM 调用、Embedding、多模态 |
| Dify | Agent 工作流、LLM 调用、Tool 执行 |
| AgentScope | Agent 调用、 LLM 调用 |
| Mem0 | 记忆增删改查、搜索 |
| MCP（Model Context Protocol） | 工具调用、资源访问 |
| Agno | Agent 调用、工具执行 |
| Google ADK | Agent 调用、工具执行 |
| LangChain | Chain/Agent 执行、LLM 调用、检索 |
...
Github地址：https://github.com/alibaba/loongsuite-semantic-conventions/。如果你正在构建 AI 应用、关心可观测性，欢迎试用、反馈与贡献：LoongSuite GenAI SemConv 以及对应探针实现，欢迎加以下钉钉群进行交流：

| Loo ngSuite GenAI SemConv | LoongSuite Java | LoongSuite Python | LoongSuite Go |
| --- | --- | --- | --- |
| 群号：174735013045 | 群号：170080021991 | 群号：101925034286 | 群号：102565007776 |

---
原文链接：https://mp.weixin.qq.com/s?__biz=MzIzOTU0NTQ0MA%3D%3D&mid=2247559954&idx=1&sn=0bb0ae4137b4f734ca75997db54e418a&chksm=e893450870ca6dbd11b5a2b16fc4de321d8b1e9473bede4bce422709727001150107fd638d8d
