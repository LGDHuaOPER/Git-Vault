...
| 指标 | 定义 |
|------|------|
| 硬规则通过率 | 所有确定性规则通过的用例占比 |
| 事实违规率 | 经确认的编造或矛盾占比 |
| 越界承诺率 | 承诺实时价格、库存等占比 |
| 结束后继续率 | 明确结束后仍营销的占比 |
| 主动微信率 | 用户未提微信却主动推荐的占比 |
| 敏感信息泄露数 | 日志和报告中手机号等泄露数量，目标为 0 |
...
### bad case 迭代闭环
一个失败样本进入永久回归集前必须满足：可稳定复现、期望行为明确且经确认、根因明确、能代表一类问题、已完成脱敏 ^[extracted]。修复后不仅保留原 bad case，补充：原问法 + 语义等价问法 + 近邻反例 + 必要的多轮版本。

### 报告设计原则
每项指标都可下钻到具体用例、请求、返回、断言和 Judge 理由——没有证据链的总分无法指导修复 ^[extracted]。报告含三类读者：产品负责人（能否发布）、算法开发（哪层哪类哪条规则失败）、测试标注（请求/返回/断言/Judge 依据）。
